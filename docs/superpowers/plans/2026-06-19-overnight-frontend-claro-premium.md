...
- 2026-06-20 (retoma) · Claude · **Corrección de sobre-declaración:** `src/components/SpanishBHistoryView.tsx` figuraba como ✓ en §5 pero solo había recibido fixes de accesibilidad (11:04); seguía 100% legacy (≈22 usos de `font-serif` en chrome, notas/puntuaciones en `text-primary` = navy global, banner de detalle `bg-primary`). Migrado de verdad con una **raíz scopeada `#spanishb-hist-root`** que remapea solo en ese subárbol `--primary` → índigo `L.primary` y `--font-serif` → IBM Plex Sans (patrón `<style>` scopeado, sin tocar `styles.css` global). Verificado que los componentes anidados ya migrados (`SpanishBPaper2HistoryView`, `SpanishBOralHistoryView`, `PanelLogros`) no usan esos tokens, así que el remapeo no los afecta. La respuesta anotada del alumno conserva serif vía `landingFontSerif` inline (texto de alumno = serif intencional, regla Claro). Sin tocar lógica, Supabase, i18n ni navegación entre vistas. prettier/eslint/tsc/build verdes. Commit mío y reversible.
