...
- [DONE] `src/routes/reservar-sesion.tsx` (ruta de reserva migrada con root scoped, header claro, slots/formulario/BookingCard Claro premium)
...
- [DONE] `src/components/SiteHeader.tsx` — verificado sin cambios: la variante `claro` existente cubre marca/header y las rutas migradas la consumen dentro de roots con variables locales; no hizo falta extenderla.
...
- 2026-06-20 12:02 CEST · Codex (yo) cerró la verificación transversal de `src/components/SiteHeader.tsx`: la variante `claro` existente ya cubre las rutas migradas y no hizo falta editarla. `src/routes/__root.tsx` se mantiene SKIP por riesgo de cascada global según el plan. Commit solo de documentación, mío y reversible.

### Segunda pasada — componentes hoja compartidos (Claude, día siguiente)
> Auditoría tras retomar: las 32 rutas quedaron migradas/verdes/pusheadas, pero quedaban **componentes hoja** que renderizan DENTRO de rutas ya migradas y seguían en estilo legacy (navy `--primary` + serif), creando choque visual. El inventario §5 los daba por cubiertos de forma optimista. Esta pasada los cierra de verdad, unidad por unidad. Las líneas siguientes son de **Claude** (no Codex).

- 2026-06-20 (retoma) · Claude · `src/components/SelectorNivel.tsx` → Claro premium: el chip de nivel activo pasaba de navy legacy (`bg-primary` global) a índigo Claro `L.primary`; etiquetas SL/HL·NM/NS en IBM Plex Mono con tracking y `tabular-nums`, contenedor cálido `L.bg2` con hairline `L.line`. Estilos inline (el componente se reusa en muchas raíces, sin depender de CSS scopeado). Sin tocar props, `courseKey`, labels ni `onChange`. prettier/eslint/tsc/build verdes. Commit mío y reversible.
- 2026-06-20 (retoma) · Claude · **Corrección de sobre-declaración (la mayor):** el panel logueado `DashboardPage` dentro de `src/routes/index.tsx` NO estaba migrado — §5 daba "index" por hecho pero solo cubría el `LandingPage` de deslogueado. El home que ve el alumno cada sesión usaba `<div className="min-h-screen bg-background">` + `<SiteHeader />` por defecto (no `claro`), titulares y nota estimada en serif, e índigos en navy (la ruta no remapeaba `--primary`). Migrado con `#dashboard-root` + `dashboardRootStyle` (mismo `CSSVarStyle` que `historial.tsx`: lienzo cálido, tokens shadcn → Claro, `--primary` → índigo, y además `--font-serif` → Plex Sans porque el panel no tiene texto literario), `<SiteHeader claro />`, focus-ring índigo scopeado, y el CTA del nudge de criterio débil pasa de botón ámbar a índigo (patrón prueba-1: banner ámbar + CTA índigo). Iconos de categoría (azul/esmeralda/púrpura) y gamificación se conservan como identidad. `head()` ya cargaba `LANDING_FONT_LINK`. Sin tocar datos, Supabase, navegación por rol ni i18n. prettier/eslint/tsc/build verdes. Commit mío y reversible.
- 2026-06-20 (retoma) · Claude · `src/components/CreditGate.tsx` → Claro premium: el diálogo de confirmación de gasto de créditos se portea a `<body>`, así que su CTA `AlertDialogAction` salía con el `--primary` navy legacy. Añadida raíz scopeada `#credit-gate-content` con `<style>` que remapea `--primary` → índigo (CTA "Sí, continuar"/"Comprar créditos" ahora índigo) + `landingFontSans` en el contenido. Icono de marca a `L.amber`; el texto pequeño ámbar del coste y el `CreditCostBadge` suben a `#9A5E10` (amberDeep, AA) en claro conservando `dark:text-amber-400`. Iconos decorativos con `aria-hidden`. Sin tocar lógica de saldo, navegación a compra ni callbacks. prettier/eslint/tsc/build verdes. (Dejo `CreditsBalance.tsx` sin tocar a propósito: vive en `SiteHeader` en contextos claro y no-claro/dark; su píldora neutral es agnóstica al tema y forzar tokens Claro arriesga regresión en dark.) Commit mío y reversible.
- 2026-06-20 (retoma) · Claude · **Corrección de sobre-declaración:** `src/components/SpanishBHistoryView.tsx` figuraba como ✓ en §5 pero solo había recibido fixes de accesibilidad (11:04); seguía con ≈22 usos de `font-serif` en chrome (titulares, notas, puntuaciones) en serif Libre Baskerville. (El `text-primary` de las notas ya salía índigo porque `historial.tsx` remapea `--primary` en su root; el clash real era la serif, que **ninguna** ruta remapea.) Migrado con una **raíz scopeada `#spanishb-hist-root`** que en ese subárbol remapea `--font-serif` → IBM Plex Sans y reafirma `--primary` → índigo `L.primary` (auto-suficiente aunque se renderice fuera de historial); patrón `<style>` scopeado, sin tocar `styles.css` global. Verificado que los componentes anidados ya migrados (`SpanishBPaper2HistoryView`, `SpanishBOralHistoryView`, `PanelLogros`) no usan esos tokens, así que el remapeo no los afecta. La respuesta anotada del alumno conserva serif vía `landingFontSerif` inline (texto de alumno = serif intencional, regla Claro). Sin tocar lógica, Supabase, i18n ni navegación entre vistas. prettier/eslint/tsc/build verdes. Commit mío y reversible.

## 9. Segunda pasada (Claude) — estado real tras retomar

Tras retomar el trabajo de Codex: las 32 rutas estaban migradas/verdes/pusheadas, pero una auditoría encontró superficie que renderiza DENTRO de rutas migradas y seguía legacy, más **dos sobre-declaraciones** en §5. Cerrado en 4 commits revisables y reversibles (todos `Co-Authored-By: Claude`), árbol limpio y alineado con `origin`.

**Migrado/corregido por Claude (cada uno su commit + gate prettier/eslint/tsc/build verde):**
- `SelectorNivel.tsx` — chip activo navy→índigo, etiquetas mono. (`4b74261`)
- `SpanishBHistoryView.tsx` — *sobre-declarado ✓*: seguía con serif de chrome. Raíz scopeada `--font-serif`→Plex Sans. (`5c75b34`)
- `CreditGate.tsx` — diálogo porteado a `<body>`: CTA navy→índigo vía remap scopeado + texto ámbar a AA. (`a0ab728`)
- `index.tsx · DashboardPage` — *la mayor sobre-declaración*: el home logueado NO estaba migrado (header default, serif, navy). Raíz `#dashboard-root` Claro completa + `SiteHeader claro`. (`4e36376`)

**Dejados a propósito (no son clash; con razón):**
- `CreditsBalance.tsx` — vive en `SiteHeader` en contextos claro y no-claro/dark; píldora neutral agnóstica al tema; forzar Claro arriesga regresión en dark.
- `gamificacion/*` (BarraXP, TarjetaRacha, LogroCard, PanelLogros, ToastLogro) — color de juego (amarillo XP, naranja racha, categorías); identidad lúdica, como el arte SVG del juego.
- `GraficoPlan/GraficoProgresion/GraficoProgresoIB` — data-viz; color funcional.
- `DevLogPanel.tsx` — solo desarrollo, no UI de producción.
- `AvatarProfesorVideo.tsx` — el vídeo es contenido (ya decidido en la 1ª pasada).
- `TextoAnotado.tsx` — `#profesor-alumno-root` ya remapea `--primary` (su micro-etiqueta sale índigo) y su cuerpo serif es texto de alumno (correcto). Único residual: el título serif de un diálogo de anotación porteado (profesor, trivial).

**Verificación cruzada:** ya no quedan `<SiteHeader />` por defecto ni roots `min-h-screen bg-background` sin migrar; las rutas que no remapean `--primary` (asignaturas/login/profesor/profesor-alumnos) usan tokens inline sin fugas navy. `__root.tsx` sigue SKIP.

- 2026-06-20 (retoma) · Claude · **Cierre del 100% legacy:** migrados los dos últimos micro-residuos serif (ninguna ruta remapea `--font-serif`, así que seguían en Libre Baskerville). `MdProse.tsx` — `h1`/`h2` del Markdown generado (feedback en paneles/historial) pasan de `font-serif` a sans heredado (Plex Sans dentro de las rutas Claro); el cuerpo ya era sans. `TextoAnotado.tsx` — el título del diálogo de anotación deja `font-serif`; el fragmento de texto del alumno (línea 273) conserva serif intencional. Solo chrome; sin tocar lógica. prettier/eslint/tsc/build verdes. Commit mío y reversible. **Con esto no queda chrome legacy: lo único no-Claro son los primitivos `components/ui/*` (token-driven por diseño) y los componentes de identidad/función ya documentados como leave.**
