...
- Edge Function `evaluate-analysis` llama a `claude-opus-4-7` con prompt caching y registra uso en `llm_uso`. En Prueba 1 devuelve la evaluación básica: bandas A/B/C/D, justificaciones, total, nota IB, comentario global, fortalezas y áreas de mejora.
- Panel de evaluación: el resultado inicial muestra texto literario, solución sin marcas, resultado, criterios, comentario global, fortalezas y áreas de mejora. El botón "Dame feedback completo" llama a `generate-analysis-feedback` y muestra el juego del Quijote durante la espera; al volver, se anclan anotaciones estructurales y de lenguaje (`AnalisisAnotado.tsx`, `FeedbackEstructural.tsx`) y se dispara automáticamente `generate-band5-essay` para el ensayo elevado a banda alta (`EnsayoBanda5.tsx`).
...
- **MVP de Prueba 2** (2026-04-30; arquitectura bajo demanda replicada el 2026-05-03) — Módulo separado de Prueba 1. Ruta `/prueba-2` (formulario: pregunta, obras, notas opcionales, ensayo) + `/historial-prueba-2`. Edge Function `evaluate-paper2` con prompt caching, tool_choice forzado, 5 criterios (A, B1, B2, C, D), justificaciones, comentario global, fortalezas y áreas de mejora; los campos ampliados (`diagnostico_comparativo`, `anotaciones`, `sugerencias_reescritura`, `ensayo_banda_5`) quedan `NULL` en la evaluación inicial. El botón "Dame feedback completo" llama a `generate-paper2-feedback`; solo entonces se muestran diagnóstico comparativo, ensayo anotado, reescrituras silenciosas (`generate-rewrite-suggestions-p2`) y ensayo elevado automático (`generate-band5-essay-p2`). Tabla `evaluaciones_prueba2` con RLS. Cuota diaria independiente (`reservar_cuota_prueba2`) para evaluar y cuota propia para feedback completo.
- **MVP del Oral Individual** (2026-05-01) — Módulo completo para el TOI (Trabajo Oral Individual). Dos modalidades: `taught` (10 min + 5 min preguntas) y `self_taught` (15 min continuo). Rutas `/oral` (formulario + guía pedagógica) + `/historial-oral`. Edge Function `evaluate-oral` con prompt caching, tool_choice forzado, 4 criterios (A/B/C/D /10 = 40 total), estimación de duración (135 ppm), diagnóstico del asunto global / equilibrio entre obras / estructura, preguntas del profesor (taught) o zonas de desarrollo (self_taught). Tabla `evaluaciones_oral` con RLS y columna generada `puntuacion_total`. Cuota 5/día (`reservar_cuota_oral`). `notaIBOral` con umbrales propios. Guía pedagógica estática con 5 campos de indagación IB, ejemplos de introducción y minutajes. Historial portal actualizado con tercer bloque Oral.
